/* Show Xplore: patch /system_data/priv/mms/app.db so the PS4-Xplorer tile
 * (LAPY20009) is shown again (visible 0 -> 1). Patches EVERY tbl_appbrowse_*
 * table (multi-account).
 *
...
int _main(struct thread *td) {
  ...
  pop("Show Xplore: running");

  db_fd = open(APP_DB, O_RDWR, 0);
  if (db_fd < 0) { pop("Show Xplore: open FAILED (%d)", db_fd); return 0; }

  unsigned char hdr[100]; read(db_fd, hdr, 100);
  page_sz = (hdr[16] << 8) | hdr[17]; if (page_sz == 1) page_sz = 65536;
  file_sz = lseek(db_fd, 0, 2);
  pop("Show Xplore: db open, page_sz=%d size=%llu", page_sz, (unsigned long long)file_sz);

  n_roots = 0;
  walk(1, 0, find_root_visit);
  pop("Show Xplore: found %d tbl_appbrowse tables", n_roots);
  if (n_roots == 0) { pop("Show Xplore: no tbl_appbrowse table"); close(db_fd); return 0; }

  int patched = 0;
  for (int r = 0; r < n_roots; r++) {
    table_root = roots[r];
    if (table_root <= 0 || table_root > 4096) { pop("Show Xplore: table %d bad root %d, skip", r, table_root); continue; }
    pop("Show Xplore: table %d root=%d", r, table_root);
    n_hits = 0;
    walk(table_root, 0, target_visit);
    pop("Show Xplore: table %d found %d target rows", r, n_hits);
    if (n_hits < 1) { pop("Show Xplore: table %d no LAPY, skip", r); continue; }
    int tpatched = 0;
    for (int i = 0; i < n_hits; i++) {
      int off = hits[i].file_off, cur = hits[i].cur_type;
      if (cur == 8) { /* hidden (serial type 8) -> restore visible=1 (9) */
        lseek(db_fd, off, 0); { char z = 9; write(db_fd, &z, 1); }
        pop("Show Xplore: #%d visible 8->9 @%d", i, off); tpatched++;
      } else if (cur == 1) { /* type 1 with value 0x00 -> restore value 0x01 */
        lseek(db_fd, off + 1, 0); { char z = 1; write(db_fd, &z, 1); }
        pop("Show Xplore: #%d visible val->1 @%d", i, off + 1); tpatched++;
      } else if (cur == 9) {
        pop("Show Xplore: #%d already visible", i); tpatched++;
      } else {
        pop("Show Xplore: #%d unexpected type %d @%d", i, cur, off);
      }
    }
    patched += tpatched;
  }
  ...
  close(db_fd);
  pop("Show Xplore: done patched=%d fsync=%d", patched, fs);
  return 0;
}
